...
5. Create a Pull Request (PR) that links back to the original issue.
   ...
   - **PR/commit message formatting:** write prose as flowing single-line paragraphs and single-line list items. Do **not** hard-wrap lines with manual newlines inside a paragraph or list item — single newlines render as soft breaks and make the raw text and diffs hard to read. Keep blank lines only between blocks (headers, paragraphs, list items).
