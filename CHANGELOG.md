...
- `KeyringSecretStore` implements the `SecretStore` trait over the `keyring` crate — macOS Keychain, Linux/BSD Secret Service (gnome-keyring / KWallet over D-Bus), and Windows Credential Manager. Each connector slug is one OS entry (keyring service `mimir`, account = slug) holding the same serialized `SecretBundle` JSON the file store writes, so bundles move losslessly between backends.
