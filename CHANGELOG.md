...
- **Tests.** 12 new unit tests for the ledger policy, 4 new cascade tests (bounded-failure → terminal, success-within-budget settles, `max_attempts: 1` fails terminal immediately, restart-resume from a captured `__durable_state` without an IMAP re-fetch), a supervisor-cycle persistence test, an instantiate injection test, a config default/override test, and a knowledge-graph `update_durable_state` facade test.
