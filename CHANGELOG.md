...
- **`owner_name` demoted to fallback.** The per-instance `owner_name` config field (defaulting to the connector slug) is now used only when no identity is injected, mirroring the Calendar connector's `None`-identity fallback; a library without a configured `[identity] name` still produces facts.
