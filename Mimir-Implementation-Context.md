...
- **Connector row projection (#560, v0.163.1):** connector registry reads and every row-returning write share one compile-time `connector_column_list` macro, so future `connectors` columns are projected consistently without runtime SQL construction or SQLx safe-SQL escapes. See `docs/connectors-framework.md`.
