...
- Composed into the system prompt as the final block, after the preset tone text and the shared operating directives, combined with an upcoming-events section rendered from the events overlay; each LLM-facing composition starts with a request-current `Now:` RFC 3339 UTC + weekday anchor (issue #472), while the condensation cache stays timestamp-free
...
- Frozen per session: native and OpenAI sessions keep the core system-prompt block captured at session creation, while the leading `Now:` anchor is replaced with a request-local value on every request; the native route's explicit `incognito: true` turns build a fresh prompt, and the OpenAI surface has no incognito path (issues #472, #473)
...
- **Obsidian-compatible export/import (#62, v0.148.0):** `mimir kb export` renders the graph as one Markdown document per entity (YAML frontmatter + wiki-links + `Dates`/`Relationships`/`Preferences`/`Facts` sections) to `--dir` → `knowledge.export_dir` → `~/AgentKnowledge` via daemon `GET /kb/export`; `mimir kb import <path>` (daemon `POST /kb/import`, loopback-gated) parses a vault through the shared grammar (`mimir-knowledge/src/obsidian/`), resolves `entity_id` or the canonical name chain (issue #182), and applies facts through `normalize_and_insert` with `source_type=Import` (0.80 confidence unless the file overrides), skipping exact existing triples, with `--dry-run` planning without writing. Object resolution uses a per-import canonical-name/object-type cache with rename invalidation (#561). Format spec: `docs/obsidian-export-import.md`; user guide: `docs/wiki/obsidian-export-import.md`.
- **Entity semantic dedup (#282):** the nightly `entity_semantic_dedup` pass caps candidate pairs (same-type entities sharing an alias or equal/contained names, excluding pairs the LLM already evaluated) and evaluates them with a strict `evaluate_entity_dedup_candidates` tool schema; Rust-side validation restricts writes to actual candidates, schema-valid actions (`merge`/`keep_separate`), and finite confidences in `[0,1]`, and rows are stored id-ordered so the `entity_merge_queue` UNIQUE constraint cannot be bypassed (re-evaluation enriches pending rows via upsert). Entities are never auto-merged: `mimir kb merges list` shows pending suggestions (loopback-gated `GET /kb/merges`) and `mimir kb merges apply`/`keep` resolve them through the existing `auto_merge_pair` logic or a `KeptSeparate` resolution (`POST /kb/merges/{id}/apply` / `keep`).
- Nightly optimization: deduplication, fact semantic dedup, entity semantic dedup, contradiction resolution, dormant cleanup.
- **Events & reminders (#74, v0.57.0):** a lifecycle + recurrence overlay on facts. A future-dated fact is a one-time event; a recurring fact (e.g. a birthday) is a recurring event; a `requires_user_action` fact is a task. An `events.upcoming_scan` job (default 06:00 & 18:00) derives overlays, auto-completes past one-time events, and advances recurring events — since v0.152.0 the overlay also carries the recurrence interval and effective series end (migration 058: `events.recurrence_rule` / `recurrence_interval` / `recurrence_until`), so a fortnightly event advances every two weeks and a bounded series stops after its last occurrence. Upcoming events surface in the "Upcoming" memory section. `entity_dates` is deprecated and removed (replaced by this overlay; recurrence logic moved to `models::recurrence`).
...
- **Connector row projection (#560, v0.164.0):** connector registry reads and every row-returning write share one compile-time `connector_column_list` macro, so future `connectors` columns are projected consistently without runtime SQL construction or escaping. See `docs/connectors-framework.md`.
