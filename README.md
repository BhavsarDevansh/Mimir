...
- **Connectors (Phase 3, in progress)** — A pluggable service ingestion framework (`mimir-connectors`) that will sync email, calendar, and photo libraries into the knowledge graph as connector-provenanced facts. The crate, feature flags (`photos`/`calendar`/`gmail`), the DB-access boundary (via the `KnowledgeGraph` facade only, no direct `sqlx`), the `connectors` instance-registry table + facade methods (sync cursor, auth state, health), the `sources.connector_instance_id` provenance FK linking facts to their connector instance, the shared `normalize_and_insert` ingestion boundary (so connectors funnel through the same confidence/corroboration/sensitivity pipeline as chat), the full entity-resolution chain — exact name → alias → FTS5 fuzzy (score ≥ 0.9) → create new, type-filtered so noisy connector data never cross-merges (Issue #182), and the runtime `Connector` trait + data types (Issue #183) — the async, object-safe contract every connector implements with two-step DB-free ingestion (`sync` → `extract` → supervisor-owned `normalize_and_insert`) — and the `ConnectorRegistry` + multi-backend factory dispatch (Issue #184), which maps each `(connector_type, backend)` pair to a `ConnectorFactory` so new backends register with no schema change, many backends coexist under one type, and reliability stays per-type — and the `ConnectorSupervisor` supervised lifecycle (Issue #185), which owns one supervised background task per `Active` connector and centralises spawn-on-startup, restart-with-backoff, a circuit breaker, auth-expiry pausing, graceful shutdown, and cursor persistence (each cycle runs `sync` → `extract` → `normalize_and_insert` in an isolated sub-task so a connector panic is contained) — and manual sync triggering (Issue #186), whereby `ConnectorSupervisor::trigger_sync` preempts a connector's polling interval with caller-supplied options (`--full`/`--since`), serialises concurrent triggers per connector via a one-permit semaphore, and returns the cycle's outcome — are in place; and the connector secret store (Issue #187) — a single `SecretStore` trait + `SecretBundle` enum (OAuth 2.0 / API token / app password) + `FileSecretStore`, one `0600` JSON file per connector under `~/.local/share/mimir/secrets/`, plaintext at rest, fail-closed permission checks, atomic writes, and slug validation — is in place; and the shared rate-limit + retry/backoff primitives (Issue #189) — a per-instance `RateLimiter` (a `governor` GCRA token bucket for requests-per-second + burst) with an optional rolling 24h daily quota that returns a non-blocking `QuotaExhausted` so the supervisor pauses gracefully, plus a `retry_with_backoff` helper for uniform 429/502/503/504 handling with exponential/linear/fixed backoff + jitter honouring a server `Retry-After` (connector LLM calls are exempt per decision D′ and route through the shared `LlmWorkerPool`) — and the configurable, always-compiled mock connector test harness (Issue #190) — a config-driven in-memory connector (`MockConnector`) that emits canned `NormalizedFact`s in both polling and push modes with health/auth/failure/panic injection, and is the T1 sync→extract→insert→query vehicle (the real `ConnectorSupervisor` + `KnowledgeGraph` ingest a mock's canned facts end-to-end with connector provenance, with no real service) — are in place; and the geocoder service (Issue #191) — a pluggable `Geocoder` trait (forward address → coords, reverse lat/lon → place) with an OSM Nominatim default backend: the trait + types live in `mimir-core` (so the Location Search tool #98 can name it without a dependency cycle) and the `NominatimGeocoder` backend lives in `mimir-connectors`, reusing the F12 `RateLimiter` (`nominatim()` ≤ 1 req/s) and `retry_with_backoff` for 429/502/503/504 + transport failures; a successful no-match yields `Ok(None)` and transport/decode failures yield `Err(GeocodeError)` (logged, never a panic), with configurable endpoint/User-Agent/contact-email (self-hosted Nominatim supported) — is in place; and the entity-locations write path (Issue #193) — a "where" fact carries a typed `NormalizedLocation` overlay that `normalize_and_insert` turns into an `entity_locations` row for the subject entity, geocoding the missing half via the injected `Geocoder` (address → coords or coords → place), with temporal bounds that model moves (home 2020–2023, home 2023–present) by superseding the prior open-ended location, and a new `source_fact_id` FK linking each row to its originating fact — is in place; proximity queries (`find_nearby`, #194) are in place (Phase 3 S4 — a SQLite bounding-box pre-filter backed by a composite coordinate index, then an exact Haversine post-filter in pure Rust, with optional temporal scoping; closes the query half of #65); the sensitive-fact confirmation path is a follow-up; the daemon wiring, optional OS-keyring backend (#188), and backend implementations land in later Phase 3 issues
