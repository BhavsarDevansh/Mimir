...
- **Connectors (Phase 3, in progress)** — A pluggable service ingestion framework (`mimir-connectors`) that will sync email, calendar, and photo libraries into the knowledge graph as connector-provenanced facts. The crate, feature flags (`photos`/`calendar`/`gmail`), the DB-access boundary (via the `KnowledgeGraph` facade only, no direct `sqlx`), the `connectors` instance-registry table + facade methods (sync cursor, auth state, health), the `sources.connector_instance_id` provenance FK linking facts to their connector instance, and the shared `normalize_and_insert` ingestion boundary (so connectors funnel through the same confidence/corroboration/sensitivity pipeline as chat), and the full entity-resolution chain — exact name → alias → FTS5 fuzzy (score ≥ 0.9) → create new, type-filtered so noisy connector data never cross-merges (Issue #182) — are in place; backend implementations land in later Phase 3 issues
