...
- New `connectors` table: `id INTEGER PRIMARY KEY AUTOINCREMENT`, `connector_type_id` FK, `slug TEXT NOT NULL UNIQUE`, `backend TEXT NOT NULL`, `display_name`, `config_json`, `status`, `auth_state`, `sync_cursor TEXT`, `last_sync_at`, `last_error`, timestamps. (No text IDs — integer PKs per project convention; slug is a label, not a key.)
- `sources.connector_id TEXT` → `connector_instance_id INTEGER REFERENCES connectors(id)` (safe migration; column always NULL today). `connector_type_id` retained. `Source` Rust struct updated.
...
- **#69 kb heatmap/reset** → standalone, deferred CLI polish.
