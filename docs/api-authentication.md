...
Before this change the API had no authentication: the loopback guard (`require_loopback`) only restricted a subset of routes to loopback callers, and loopback is not an auth boundary. Any local process — or any other local user on a multi-user machine — could read the entire knowledge graph, forge chat turns, edit or forget facts, and delete connectors. A `server.bind_addr = "0.0.0.0:8080"` configuration exposed every endpoint to the network with no protection at all.

The bearer token closes that gap: without the token, the knowledge graph is unreadable and unmutable, regardless of whether the caller is local or remote. The loopback guard remains as a second, independent layer for destructive routes, and the daemon logs a warning at startup when the bind address is not loopback (the token is then the only access control).

## Token Lifecycle

- **Location:** `~/.local/share/mimir/api_token` (resolved via `mimir_core::paths::data_dir`, so `XDG_DATA_HOME` is honoured).
- **Generation:** 32 random bytes (256 bits) from the operating system's CSPRNG (`getrandom`), hex-encoded to 64 characters. Created with `create_new` semantics and mode `0600` (Unix) so a concurrent creation race resolves to whichever process wrote first and both processes then read the same token.
...
- `require_auth` extracts the `Authorization` header, strips the `Bearer ` prefix, and compares the presented token against `AppState::api_token` in constant time (`mimir_core::auth::verify_api_token`, backed by the `subtle` crate) so a wrong token cannot be distinguished from a right one by timing.
