...
Open performance issues tracked by these benchmarks: #523 (CLI stop sleep), #524/#525 (SQLite pragmas), #526 (fact-insert overhead), #529 (alias batch insert), #532–#534 (fixed sleeps in tests), #535 (migration template), #536 (hook-exit timeout), #537 (supervisor poll loop). The daemon-guard start timeout is now injectable for tests (#530), and the LLM-client retry schedule is injectable, so retry tests no longer wait through production backoff (#531).
