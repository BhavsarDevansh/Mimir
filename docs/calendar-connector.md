...
- **`<event> located_in <place>`** — the `LOCATION` resolves to a `Place` entity via F5. The venue is a property of the event, not the user's location history, so this fact carries no `entity_locations` overlay (a calendar full of meetings would otherwise bloat `Visited` rows).
- **`<attendee> attending <event>`** — each `ATTENDEE` resolves to a `Person` entity via F5 (the `CN` parameter, else the `mailto:` value).
