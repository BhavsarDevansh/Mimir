...
5. **Cursor** — deletions arrive inside the sync-token incremental window, so no new cursor is needed; the tombstone is drained in the same cycle it is reported. A trash failure aborts the cycle before the cursor is persisted, so a restart resumes from the old sync-token and re-reports the deletion (an in-process retry does not re-fetch — the in-memory token has already advanced; tracked as #314).
