...
Destructive and sensitive routes are additionally loopback-only: the `require_loopback` middleware in `mimir-server/src/app.rs` rejects non-loopback callers with `403 Forbidden` before the handler runs, so a daemon bound to a LAN address never accepts remote mutations even from a caller that holds the token. Since issue #25 the middleware extracts the transport-independent `LocalPeer` connect-info type: Unix-socket peers are always local (the socket file's mode `0600` filesystem permissions are the access control), while TCP peers must still be loopback addresses. Gated routes: `POST /memory/refresh`, `POST /kb/optimization/run-now`, `POST /kb/facts/forget`, `POST /kb/facts/{id}/confirm`, `POST /kb/facts/{id}/reject`, `GET /kb/pending`, `GET /kb/staged`, `POST /kb/staged/{id}/map`, `POST /kb/staged/{id}/reject`, `DELETE /kb/trash`, `POST /kb/trash/restore`, `POST /connectors/{id}/tokens`, `POST /connectors/{id}/forget`, and `POST /stop`. The auth layer runs outside the loopback layer, so an unauthenticated non-loopback caller gets `401` before the `403` loopback check.
...
On `/v1/chat/completions` streaming, a provider failure on the first stream attempt returns `500` with the flattened, length-bounded failure detail in the OpenAI error message (e.g. an upstream `503` overload), while queue-full keeps its `503` + `Retry-After` response; failures after the stream starts are carried in the terminal `event: error` SSE frame (PR #490 review).
