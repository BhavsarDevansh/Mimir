...
- **HTTP client mode (current)**: every client-mode command talks to the daemon over HTTP via `mimir-client`; the daemon owns the knowledge graph, memory, connectors, and LLM pool. `mimir start` runs the Axum server in-process; `mimir-client` is the single transport for client commands (`kb` and `connector` share the `cli_util` helpers). Local commands prefer the Unix domain socket (`~/.local/share/mimir/mimir.sock`, issue #25) and fall back to TCP for remote daemons (`MIMIR_BASE_URL`) and Windows — see `docs/uds-transport.md`.
