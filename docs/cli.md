...
- Display-name conversion uses `mimir_core::tools::snake_to_title_case`, the same helper used by tool labels (issue #622).
...
- `mimir connector auth <slug> [key=value...] [--config-json <json>] [--password <p> | --token <t> | --password-stdin | --token-stdin] [--json]` — ingest credentials for an existing instance (completes an unauthenticated `add`, or re-auths after expiry) without `remove` + re-`add`; the kind comes from the flags, the `MIMIR_CONNECTOR_*` env vars (exactly one set), an interactive selection, or the `auth.kind` of a re-supplied config, and the secret resolves with the same flag → stdin → env → prompt precedence as `add` (issue #270). An `auth.kind=oauth` config runs the interactive PKCE loopback flow (A4 / #205) instead of prompting; when no kind is re-supplied, the daemon exposes the stored non-secret auth config so an OAuth connector re-runs the PKCE flow from its stored endpoints (`auth.auth_uri` / `auth.token_endpoint` / `auth.client_id` / `auth.scopes`) — re-auth after expiry needs no re-supplied config (issue #507; config-free re-auth serves public PKCE clients, while a confidential client re-supplies `auth.client_secret` with the config — kept in the credential bundle, never in `config_json` — and the daemon persists the driving non-secret OAuth slice into `config_json` alongside the bundle). The interactive kind selection also offers an "OAuth 2.0" fallback, which requires the OAuth fields as config pairs when the stored config does not carry them.
...
Review facts whose LLM predicate did not resolve to the closed relationship taxonomy (issue #468): `mimir kb staged list [--json]` lists unmapped rows from the loopback-gated `GET /kb/staged`; `mimir kb staged map <id> --relationship-type-id <leaf-id> [--note ...]` maps a row to an existing emit-eligible leaf via `POST /kb/staged/{id}/map`; `mimir kb staged reject <id> [--note ...]` marks the row rejected via `POST /kb/staged/{id}/reject`. Mimir does not auto-create or auto-drop predicates — mapping and rejection are explicit review actions.
