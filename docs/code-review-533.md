...
| Test reliability | Rapid agent-task exits coalesced `Notify::notify_one` permits, so a second task exit could be missed. | Medium | Use monotonic watch sequences, subscribe before submission, and await exact exit counts. |
...
| DRY compliance | Hook and scheduler tests duplicated the same bounded watch-sequence wait. | Low | Move the helper into the crate-local test-only `test_sync` module and reuse it for hooks, scheduler, agent-runtime, and worker-pool tests. |
