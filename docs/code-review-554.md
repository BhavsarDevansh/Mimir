...
| VISION compliance | Provenance, privacy, and pin/deprioritization are only placeholders at the aggregate-view level. | Medium | Recorded their explicit current states and documented #581, #582, and #284 as the follow-up issues that populate them. |
...
- `cargo bench -p mimir-knowledge --features test-benchmark --bench memory_benchmark` completes with no violations. Local metrics: precision@5 `0.8`, recall@5 `1.0`, provenance accuracy `1.0`, temporal correctness `1.0`, privacy false allow/block `0.0`, retrieval p95 `241.0 µs`, retrieval p99 `362.0 µs`, ingestion `434.986 facts/s`, wall time `254.0 ms`, token estimate `159`.
