...
- Config file changes are filtered by exact filename match: an event is acted on only when the event path's file name equals the config file's own file name (e.g. `config.toml`), so a differently-named file such as `myconfig.toml` is never watched.
