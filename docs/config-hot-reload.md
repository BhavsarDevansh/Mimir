...
- Config file changes are filtered by exact filename match: the watcher monitors the parent directory, then ignores events whose file name does not exactly equal the config file's own file name (e.g. `config.toml`), so a differently named file such as `myconfig.toml` does not trigger a reload.
