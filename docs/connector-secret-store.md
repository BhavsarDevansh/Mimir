...
An OS-keychain backend over the `keyring` crate — macOS Keychain, Linux/FreeBSD/OpenBSD Secret Service (gnome-keyring / KWallet over D-Bus), and Windows Credential Manager. Feature-gated behind `secrets-keyring` (off by default, because headless systemd boxes often lack a Secret Service daemon) and selected at daemon startup with `secrets.backend = "keychain"` (config section `[secrets]`, env override `MIMIR_SECRETS_BACKEND`; see `docs/config-system.md`).

- **Entry mapping.** Each connector slug is one keyring entry: service `mimir`, account `<slug>`, payload the serialised `SecretBundle` JSON. Both backends round-trip the same `SecretBundle` JSON schema losslessly, so a bundle written by either store deserialises in the other — the byte sequences differ only in formatting, because the file store writes pretty-printed JSON and the keyring store writes compact JSON.
...
- **Platform notes.** On Linux/FreeBSD/OpenBSD the Secret Service backend is the pure-Rust zbus stack with `async-io` (keyring's `tokio` feature is intentionally not enabled — zbus's `block_on` then panics inside a tokio runtime). `keyring`'s `Entry` API is blocking even with `async-secret-service`, so every operation runs on a dedicated blocking worker thread (`tokio::task::spawn_blocking`), never on the Tokio executor — the pattern keyring's own docs recommend. Building the feature needs no C libraries; the `keyring` crate is pinned to 3.6.3 because 4.x requires Rust 1.88, above the workspace MSRV 1.85 (same cap as `icalendar`).
...
- **Async trait.** `SecretStore` is `#[async_trait]`, which let the OS-keychain backend (#188) implement the trait without a breaking change, and composes with the async `Connector` pipeline. The file backend does inline blocking I/O (fast for tiny JSON payloads); the keyring backend offloads its blocking OS calls to a dedicated blocking worker thread so they never stall the Tokio executor.
