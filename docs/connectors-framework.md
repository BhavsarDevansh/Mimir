...
- **`connector_fact`** (`mimir_connectors::fact`, issue #255) is the single constructor for connector facts: it fills the fixed defaults (`source_type: Connector`, non-sensitive, non-correction, no category ids, no user action) and takes the per-shape fields (subject, relationship, object, entity-ness, temporal bounds, recurrence, raw reference, extraction method, event-type hint, location overlay) as arguments. The Photos, iCal VEVENT (Calendar + Email iMIP), and Email JSON-LD backends all funnel through it, so a new connector cannot silently drift on a default. The module is cfg-gated to `any(feature = "photos", feature = "calendar", feature = "email", test)` (issue #342) so the framework core stays warning-free under `--no-default-features`.
- **Confidence** is the per-source-type / per-connector reliability score read from the `connector_reliability` table (`confidence::connector_reliability`, seeded defaults as fallback) with no extraction-method discount. Corroboration / supersession / inference are inherited from `insert_fact_in_tx`, so cross-connector corroboration (Email flight + Calendar event on overlapping dates) is an explicit acceptance criterion, not an accident.
...
- **`on_cycle_succeeded`** (issues #314, #332) is the failure-safe cursor-adoption hook: the supervisor calls it (default no-op) after a cycle fully succeeded — extraction, trashing, fact insertion, and cursor/durable-state persistence all committed — with the persisted `SyncOutcome::new_cursor`, so the connector may adopt it as its in-memory progress marker. Connectors must **not** advance an in-memory cursor inside `sync` (unless they re-deliver failed windows by other means, e.g. a durable retry ledger): the persisted cursor advances only on success, so an earlier adoption would skip a failed cycle's window on the next in-process cycle. The Calendar, Email, and Photos connectors all implement it — their `sync` no longer advances the in-memory cursor. The Email connector's durable retry ledger (issue #262) records the terminal LLM-layer failures (retries now run through the `connector_item.remember` hook, issue #386), so a hard extract/insert/persist failure still needs the un-advanced cursor to re-fetch the failed window; in IDLE (push) mode the Email connector additionally skips the IDLE wait and re-fetches from the last confirmed cursor on the next in-process cycle, because the IDLE notification for the failed window will not re-fire, and re-fetches are deduped against the staged buffer so re-fetched windows are never duplicated; the Photos connector (push mode) re-scans the watch directory from the last confirmed cursor on the next in-process cycle, because the file watcher does not re-deliver consumed events. Connectors whose progress lives solely in the persisted column leave the default.
...
The issue spec described the mechanism as a per-connector `tokio::sync::Notify` plus a serialisation semaphore. The implementation uses a small request channel (carrying the `SyncOptions` and returning the outcome) instead of a bare `Notify`, because `--full` / `--since` must reach the cycle and the HTTP route needs the sync result — but the one-permit semaphore is kept as the explicit serialisation gate, matching the spec's intent ("no concurrent sync on the same connector"). The runner's push-success arm drains the channel too, so a trigger accepted while an `auto` connector's mode is still unprobed is never stranded if the probe resolves to push before the runner reads it (issue #475).
...
| `photos` *(feature `photos`)* | `PhotosConnector` + `PhotosConnectorFactory` + `PhotosCursor`: read-only local-filesystem push connector — `notify` recursive watcher + `kamadak-exif` GPS/datetime extraction + per-file mtime/inode incremental cursor, with GPS → place reverse-geocoding via the shared `Geocoder` | C1–C2 — done (#195–#196) |
| `secrets` | `SecretStore` trait + `SecretBundle` + `FileSecretStore` + `InMemorySecretStore`: one store for every auth kind (OAuth tokens, API tokens, app passwords), keyed by connector slug, `0600`/`0700` fail-closed permissions, atomic writes, slug path-traversal validation | F10 — done (#187) |
| `rate_limit` | Shared `RateLimiter` (governor token bucket + optional rolling 24h daily quota) + `retry_with_backoff` (429/502/503/504 with jitter and `Retry-After`); connector LLM calls are exempt (decision D′) | F12 — done (#189) |
| `geocoder` | OSM Nominatim backend implementing `mimir_core::geocoder::Geocoder` (forward `/search` + reverse `/reverse`, policy-compliant ≤ 1 req/s via the F12 limiter, transient-HTTP retry) | S1 — done (#191) |
| `ical` | Shared RFC 5545 VEVENT parsing + fact extraction (`parse_ical_to_vevents` / `vevent_to_facts`): single source of truth for the Calendar CalDAV and Email iMIP backends | C4/C6 — done (#198, #200) |
| `calendar` *(feature `calendar`)* | `CalendarConnector` + `CalDavClient` (PROPFIND + sync-collection REPORT, sync-token incremental sync): Polling-mode event ingestion (`user has_event` / `located_in` / `attending`) + write-back (`act`: create/update/delete events) + tombstone propagation | C3–C4 — done (#197–#198) |
| `email` *(feature `email`)* | `EmailConnector` + IMAP transport (LOGIN / XOAUTH2, IDLE push with polling fallback, UIDVALIDITY-safe UID cursor, rustls TLS): deterministic extraction cascade (iMIP invites, schema.org JSON-LD, LLM prose) | C5–C7 — done (#199–#201) |
| `oauth` *(feature `oauth`)* | `OAuthHttpClient` (the `oauth2`-crate `AsyncHttpClient` adapter over the workspace reqwest 0.13 client), used by the A4 CLI PKCE flow (implemented, v0.97.0); plus the `refresh_token` / `resolve_access_token` refresh helpers (cfg-gated to `any(feature = "calendar", feature = "email", test)`, issues #351, #374, so the `oauth`-only combination used by the CLI PKCE flow stays warning-free) with the HTTPS/loopback endpoint gate and secret-hygiene error mapping, used by the Calendar and Email OAuth backends and their tests | #240 — done (v0.96.0) |
...
`cargo doc -p mimir-connectors --no-deps` must build with zero warnings under `--all-features` and `--no-default-features` (issue #276); `scripts/tests/rustdoc_test.sh` enforces both with `RUSTDOCFLAGS="-D warnings"` so newly-introduced broken links fail at review time. Module-level `//!` docs resolve intra-doc links against the *parent* module's scope, so links to the module's own items use full paths (`crate::ical::parse_ical_to_vevents`, `crate::registry::ConnectorRegistry::create`) or `Self::` where applicable, and external types use full paths (`mimir_knowledge::normalize::NormalizedFact`). When a label already contains its resolvable path, omit the explicit target (as in `[`SecretStore`]` rather than `[`SecretStore`](crate::secrets::SecretStore)`); rustdoc treats a redundant target as a warning (issue #522). Private items (private modules, `pub(crate)`/`pub(super)` types) are rendered as code spans (`` `config` ``), never links, so public docs never warn about private targets. Ambiguous names use namespace disambiguators (`macro@async_trait`).

## Feature flags

```toml
[features]
default = ["photos", "calendar", "email"]
test-utils = ["dep:wiremock"] # shared OAuth test doubles (issues #290, #298), off by default
...
oauth = ["dep:oauth2", "dep:http", "dep:url"] # shared OAuth 2.0 client + refresh (issue #240)
test-mock-oauth = ["oauth", "dep:rcgen", "dep:sha2", "dep:tokio-rustls", "dep:rustls", "dep:base64"] # in-process mock OAuth server (T2 / #207), off by default
calendar = ["dep:icalendar", "dep:roxmltree", "dep:chrono-tz", "dep:uuid", "oauth"] # CalDAV calendar ingestion (C3–C4 done)
email = ["dep:async-imap", "dep:base64", "dep:tokio-rustls", "dep:rustls", "dep:rustls-native-certs", "dep:futures", "dep:icalendar", "dep:chrono-tz", "dep:mail-parser", "oauth"] # IMAP email ingestion (C5–C7 done)
```

The framework core is **always built**; the mock connector and the OAuth test doubles are test harnesses gated by the off-by-default `test-mock-connector` / `test-utils` / `test-mock-oauth` features, so production builds never compile them. Running `cargo build -p mimir-connectors --no-default-features` therefore still compiles a working framework — the test harnesses and the gated backends are simply absent. The `photos` feature gates the `notify` / `notify-debouncer-full` / `kamadak-exif` dependencies and the `photos` module (C1–C2 / #195–#196); `oauth` gates `oauth2` / `http` / `url` and the `oauth` module (#240); `calendar` and `email` gate their backend deps and modules and enable `oauth` for their OAuth refresh path. Items with a narrower caller set are gated to their callers rather than their feature: like `fact` (issue #342), the OAuth refresh helpers are cfg-gated to `any(feature = "calendar", feature = "email", test)` (issues #351, #374) — the `oauth`-only combination (CLI PKCE) uses the HTTP client but not the refresh grant, so gating to the `oauth` feature alone would leave them as dead code.

Integration tests follow the same gating: each backend test file carries a crate-level `#![cfg(feature = "...")]` gate (`calendar_*_tests.rs` → `calendar`, `photos_connector.rs` → `photos`, the mock/supervisor tests → `test-mock-connector`, `oauth_pkce_e2e.rs` → `test-mock-oauth`), and the shared `tests/common/mod.rs` fixtures gate their calendar-specific imports and helpers behind `#[cfg(feature = "calendar")]` so every feature combination compiles under `--all-targets` (issue #277). `scripts/tests/no-default-features_test.sh` enforces the `--no-default-features` build matrix at review time, with `RUSTFLAGS="-D warnings"` across the whole matrix so a supported combination cannot regress to dead-code or unused-import warnings (issues #342, #351, #374). The Photos supervisor test polls for the pre-seeded closed-taxonomy predicate (`took_photo_at`) rather than implying runtime auto-creation.
...
Issue #179 added the `connectors` instance-registry table (migration `042_create_connectors.sql`) plus its Rust model, queries, and `KnowledgeGraph` facade methods. Each row is a single configured connector instance — one email account, one CalDAV calendar — so backends can persist sync cursor, auth state, and health across daemon restarts.
...
- **OS keyring** backend is implemented as #188 (feature-gated `secrets-keyring`, off by default because headless systemd boxes often lack a Secret Service daemon; selected via `secrets.backend = "keychain"`).
...
scripts/tests/no-default-features_test.sh # --no-default-features --all-targets build matrix, warning-free (issues #277, #342, #351, #374)
