...
- `transport` — The resolved [`DaemonTransport`](../mimir/src/transport.rs) (a Unix socket path, or a TCP base URL like `http://127.0.0.1:8080`). The probe is a bounded 500 ms connect attempt on the Unix socket — a local syscall that succeeds only while the daemon is listening — or `GET {base_url}/health` over TCP. Resolution order: `MIMIR_BASE_URL` → Unix socket → TCP fallback; see [CLI base URL](#cli-base-url) below.
...
| `StartTimeout` | The daemon did not respond to the transport probe (socket connect or `GET /health`) within the 10 s post-spawn polling window. |
| `Connection` | Reserved for unexpected probe-level failures (currently unused). |

## Detection Flow

1. **Fast probe** — A 500 ms probe of the resolved transport: a connect attempt on the Unix socket (a local syscall with no HTTP round trip, succeeding only while the daemon is listening), or a dedicated `reqwest` client sending `GET {base_url}/health` over TCP. `health` is a cheap liveness endpoint that never touches the LLM backend or database, so a healthy-but-slow provider cannot make the probe time out. If the probe succeeds, the helper returns `Ok(())` immediately.
...
4. **Polling** — The helper enters a poll loop:
   ...
   - On the first successful probe (socket connect or `GET /health`), return `Ok(())`.
