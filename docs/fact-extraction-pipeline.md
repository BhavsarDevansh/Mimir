...
- **Confidence** is `confidence::initial(source_type, connector_type)` — the per-source-type / per-connector reliability score with **no extraction-method discount**. Corroboration, supersession, and inference are inherited for free from `insert_fact_in_tx`, so a cross-connector corroboration (e.g. an Email flight fact and a Calendar event on overlapping dates) adds a source and boosts confidence without creating a duplicate fact.
...
The extraction prompt defines role, schema, classification criteria, a DB-driven category tree of assignable categories, DB-derived predicate standards, and a softened sensitivity instruction ("Flag health, financial, relationship, religious, political, or legal facts. Mimir will validate your assessment."). The category guide is rendered from the database rather than a separate depth-limited query, so the model sees every valid classification choice without needing a separate catalogue tool call. The synthetic structural Root is omitted because it is taxonomy infrastructure, not a fact classification, while it remains available to taxonomy administration and tree APIs; categories parented by the Root render as top-level assignable categories (#600). The predicate standards are rendered from the same DB taxonomy's emit-eligible leaves (name plus guidance text, grouped by root), so the prompt and the `remember` tool schema share one vocabulary instead of a second hand-maintained copy (#598). The prompt contains **no conditional logic, no workflow instructions, and no "if X then Y"** — all of that lives in Rust.
...
During extraction, each fact's `relationship_type` is resolved through `KnowledgeGraph::resolve_canonical_relationship_type`, which enforces the Rust-side canonical predicate allow-list (`CANONICAL_PREDICATES`, issue #401): the name is trimmed/lowercased (via `normalize_alias`), looked up in the `relationship_type_aliases` table — the single source of truth — and the resolved canonical type must be an emit-eligible leaf. LLM synonyms such as `attended`, `hobbies`, or `works_for` therefore map to their canonical types (`studied_at`, `hobby`, `works_at`) purely from seeded data — there is no hardcoded synonym map in code. The positive-preference family resolves only to the registered `prefers` leaf; an unregistered `favourite_movie` is rejected, and no predicate is auto-created, so an LLM-invented predicate can never pollute the ontology. The resolved canonical name then drives `split_list_objects`.

The batch flow (`extracted_to_normalized` → `normalize_and_insert`, shared by `extract_facts`/`extract_facts_with_context`) tolerates predicate-resolution errors per-fact: one invalid predicate is recorded in `ExtractionOutcome::errors` without aborting the rest of the batch. The server-side hook entrypoint is `ChatLearningHandler`'s direct `extract_facts_with_context` call (`mimir-server/src/state/hooks.rs`); `process_remember_output` remains a separate library/API path used by the on-demand Librarian pipeline.
...
>
> **Issue #401:** the conversational extraction path no longer auto-creates unknown predicates. `resolve_canonical_relationship_type` accepts only seeded canonical predicates and their aliases when the target row is emit-eligible; anything else (e.g. an LLM-invented `moved_into` or unregistered `favourite_movie`) is rejected with a `Validation` error collected in `ExtractionOutcome::errors`, and no `relationship_types` row is created. Migration `050_seed_canonical_predicates_and_reconcile.sql` seeds the predicates the extraction path legitimately uses that were missing (`skill`, `has_appointment`, and the sensitive predicates migration `029` intended to mark), and deletes auto-created types no fact references. The shared `normalize_and_insert` boundary stays permissive for connector-provenance facts, but the connector surface is now seed-pinned (issue #412): migration `053` seeds every deterministic connector-emitted predicate as canonical with constraints where applicable, `mimir-connectors` tests assert extractor output uses only canonical vocabulary, and the email LLM layer rejects non-canonical predicates, so no connector path can silently auto-create a `relationship_types` row.

## List Splitting (v0.128.0)

`split_list_objects` in `mimir-knowledge/src/extract/parse.rs` expands a fact whose object literal is a comma-separated list into one fact per item, so the LLM cramming `hobby → "Geopolitics, Software Development, Tech"` into a single fact still yields three independent `hobby` facts. The predicate allow-list is the single shared `MULTI_VALUED_PREDICATES` constant in `mimir-knowledge/src/graph/predicates.rs` (issue #405): the same constant drives the insert overlap logic in `queries/fact/insert.rs` (distinct objects for these predicates coexist instead of superseding), so the splitter and the multi-valued insert semantics can never drift apart. Legacy `favourite_food` and `favourite_colour` aliases resolve to the closed `prefers` leaf, which is multi-valued, so separate preference objects remain active. The extraction prompt keeps a single high-level rule ("Emit one fact per list item") and no longer carries the BAD/GOOD parsing example, so list-splitting is enforced deterministically in Rust rather than delegated to prompt engineering.
