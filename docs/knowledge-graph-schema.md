...
- `insert_category(pool, new, now)` — rejects a category whose `parent_id` equals its own `id` with a `Validation` error before insert, so the DB tree cannot contain a node that is invisible to root-first traversal.
