...
1. **KG-focused base** (`build_base_prompt`) — extraction rules, the DB-driven Categorisation Guide, DB-derived predicate standards, list splitting, within-output deduplication, and the output contract. The guide renders the complete category tree with indentation so every seeded or user-added assignable category remains selectable and deeper sub-categories cannot drift out of the prompt; the synthetic structural Root is reserved for tree APIs and governance surfaces, while categories parented by it render as top-level assignable categories (#600). The predicate standards are likewise rendered from the taxonomy's emit-eligible leaves (name plus DB description), so prompt and `remember` tool schema cannot drift apart (#598). Shared with the simple `extract_facts` path.
