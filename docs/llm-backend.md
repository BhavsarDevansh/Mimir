...
Consumers: `mimir-knowledge::extract::parse::parse_remember_output` (the `remember.chat` hook's conversational extraction, formerly the `remember` tool) and `mimir-connectors::email::llm::parse::parse_output` (Email C7 / #201 `extract_email_facts` tool). Both tool schemas (`remember_tool_schema`, `email_extraction_tool_schema`) are static and built once via `LazyLock`.
