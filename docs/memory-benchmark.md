...
The runner emits one JSON report with quality and performance metrics. It can save a local baseline, compare against a baseline, and fail on budget violations or regressions. Performance values are machine-specific, so committed baselines should not be treated as universal performance expectations.
...
| `privacy_false_allow_rate` | Fraction of sensitive fixtures incorrectly allowed without confirmation. |
