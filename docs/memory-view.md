...
`MemoryViewUsage` records the character count, configured limit, percentage, approximate token count, and whether the composed content is within budget with compact integer types; usage percentage is a byte that saturates at 255 when over-budget content exceeds that percentage. The budgeted rendering policy preserves a multi-paragraph condensed core, passes it separately from upcoming content, and truncates only the upcoming section when the combined content exceeds the configured limit; if core alone reaches the limit, budgeted rendering truncates core to that limit and omits upcoming. It omits the `Now:` anchor because the system-prompt composer supplies exactly one request-local anchor.
