...
The nightly optimization pipeline maintains graph health by running a fixed sequence of passes over the knowledge graph. It is implemented in `mimir-knowledge/src/optimization/mod.rs` and orchestrated by `OptimizationRunner`.
