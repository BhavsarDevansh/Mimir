...
2. **Semantic Deduplication** – sends near-match candidates to the LLM with a strict JSON schema. Auto-merges pairs with confidence >= 0.9; queues uncertain pairs in `dedup_queue`. Rust tracks each fact's current confidence so successive merges preserve earlier boosts.
