...
- Object planning uses one per-import cache of canonical entity IDs and relationship-derived object types, with entries invalidated when an entity update changes the graph during the same import; renamed names are recorded as unavailable for the rest of that import, and alias matches are resolved by the relationship-constrained object type so dry-run and apply agree, with benchmark coverage measuring repeated shared objects across files.
...
- `facts_new` counts each accepted fact line, whether the resulting fact becomes active immediately or enters confirmation; it excludes invalid predicates and exact existing triples. The hand-written `[[object]] — predicate` relationship form uses the same parser path as the canonical `predicate → object` form.
