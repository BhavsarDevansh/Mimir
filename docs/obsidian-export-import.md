...
- Object planning uses one per-import cache for canonical-name resolutions and relationship-derived object types. Cache entries are invalidated when an entity rename changes the graph during the same import, while alias matches are resolved by the relationship-constrained object type so dry-run and apply agree; benchmark coverage measures repeated shared objects across files.
