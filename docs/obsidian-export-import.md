...
- Object planning uses one per-import cache of canonical entity IDs and relationship-derived object types, with entries invalidated when an entity update changes the graph during the same import; alias matches are resolved by the relationship-constrained object type so dry-run and apply agree, and benchmark coverage measures repeated shared objects across files.
