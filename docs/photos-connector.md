...
- **Subject** — the canonical user identity (`[identity] name`, injected via
  `ConnectorContext::user_identity`) when configured, else the per-instance
  `owner_name` (`Person`); `owner_name` defaults to the connector slug. See
...
The daemon injects the canonical user identity (`config.toml`'s
...
against `owner_name` (defaulting to the connector slug) so a library still
produces facts. When both are present, the injected identity wins.
...
defaults to the slug and is used only when no canonical user identity is
injected (see [User identity](#user-identity)); `debounce_ms` defaults to
2000; `extensions` defaults to the set above. The `__slug` / `__ctype` /
`__instance_id` / `__cursor` keys are injected by the supervisor and ignored
by the connector's own serde DTO.
...
- Unit tests (`src/photos/`): cursor diffing/pruning/round-trip, path
  ...
  are generated by `tests/fixtures/gen_exif.py` (pure stdlib). Issue #246
  adds: identity-vs-`owner_name` subject precedence, the `None`-identity
  `owner_name` fallback, and a factory-level test proving
  `ConnectorContext::user_identity` reaches extracted facts end-to-end.
