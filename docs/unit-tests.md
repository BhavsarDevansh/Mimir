...
- `connector/`: 37 tests — CLI connector flows: `parse_duration` (bare seconds, units, case/whitespace, garbage, overflow), config merging (`merge_config` nesting/dotted keys/JSON base/overrides, `parse_config_scalar` quoting/arrays/objects/malformed fallback), credential-kind detection, `title_case`, server-error rendering, connector-not-running detection, and the wiremock-backed command flows (`add`, `auth`, `act`, `forget`, `pause_and_resume`, `remove`, `sync`, `resolve_connector`, OAuth PKCE config extraction and token ingest).
...
- `daemon_guard/`: 10 tests — the auto-start guard's HTTP probe, prompt, spawn, timeout, and child-env secret-stripping paths.
- `chat`: 6 tests for `format_markdown_for_terminal` fence spacing (start, middle, consecutive fences, end, empty input, no fences).
- `cli_util`: 1 wiremock test — `client_with_token` falls back to a tokenless client when the token is rejected as an invalid header value.
