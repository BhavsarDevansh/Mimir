...
49 lib tests on Unix (up from 38); non-Unix platforms have 44 because the two SIGHUP and two file-watcher regression tests in `server.rs` plus the SIGTERM regression test in `shutdown.rs` are Unix-only.
