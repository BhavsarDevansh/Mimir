...
- Inline unit tests grew from ~12 to 63 (`mimir-api-types`), ~24 to 74 (`mimir-client`), 211 to 279 (`mimir-core` lib), 110 to 195 (`mimir-knowledge` lib), 38 to 41 (`mimir-server`), and 15 to 29 (`mimir` bin); `mimir-connectors` currently carries 319 lib tests across the email/oauth/photos/supervisor/rate-limit/calendar/geocoder/ical/secrets modules (see `docs/unit-tests.md`).
