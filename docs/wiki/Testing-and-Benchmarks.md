...
- **Knowledge-graph test fixture** (`mimir-test-support`, used only as a development dependency) keeps a pre-migrated SQLite template per test binary and copies it for each standard integration test, so tests remain isolated from one another without repeating the 60-migration schema setup (issue #535).
...
- Core hook, scheduler, agent-runtime, worker-pool, context, and conversation tests now synchronise on readiness signals, monotonic task-exit and job-start sequences, explicit database timestamps, pending-state polling, or Tokio mock time instead of fixed wall-clock sleeps for timing transitions (issue #533), making the timing-window tests deterministic and faster.
- The knowledge-graph tests use a shared pre-migrated template rather than re-running the full schema for every standard fixture, reducing measured setup from 72.68 ms to 4.06 ms (issue #535).
