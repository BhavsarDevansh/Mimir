...
**The configurable mock connector test harness is in place (issue #190 / F13).** An always-compiled in-memory connector whose behaviour is driven entirely by its `config_json`: it emits canned `NormalizedFact`s on a configurable cadence in both polling and push modes, and can inject health/auth states, failures, and panics to exercise the supervisor. It is the T1 sync→extract→insert→query vehicle — the real `ConnectorSupervisor` + `KnowledgeGraph` ingest a mock's canned facts end-to-end with correct connector provenance, without any real service. See [Mock Connector](mock-connector.md). ## How connector credentials are stored
...
- **Email** — IMAP ingestion with IDLE, structured and LLM-based fact extraction (flights, bookings, contacts).
