...
The total wait time is capped at **10 seconds**, including slow probes and the delay before the next probe. If the daemon has not started by then, the command exits with a timeout error. Automated tests inject a shorter timeout without changing this normal behaviour.
