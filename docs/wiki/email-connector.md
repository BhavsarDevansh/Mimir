...
- Obvious bulk-marketing mail (sent through known marketing-sender domains like Mailchimp, Sendgrid, HubSpot) is skipped **before** any LLM call by a deterministic Rust filter, so marketing mail never costs an API call and never becomes junk facts. For everything else, the LLM simply returns an empty fact list when there is nothing to extract.
- The LLM call runs on Mimir's shared LLM **system queue** — at lower priority than your chat — so an email sync burst never slows down your conversation, and a chat you send mid-extraction jumps ahead. This is what the C7 acceptance criterion ("a queued chat preempts a waiting connector call") guarantees.
