...
- The extraction prompt lists the full **canonical predicate vocabulary** (the relationship types Mimir can store, e.g. `has_appointment`, `has_flight`, `resides_in`, `purchased_from`) instead of a few examples, so the model knows the exact words to use. A fact whose relationship type is not in that vocabulary is dropped — and the drop is **counted**, not silent.
