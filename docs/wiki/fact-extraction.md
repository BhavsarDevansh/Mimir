...
- **Category visibility**: The extraction prompt includes the complete DB-driven tree of assignable categories, so categories such as `Sweet Foods` and `Board Games` are visible even when they are grandchildren in the taxonomy. The model can therefore choose the most specific category available, while Rust still validates the selected IDs and applies a deterministic category fallback when needed. The synthetic structural Root is not offered as a fact classification, and categories parented by it are rendered as top-level assignable categories (#600).
- **Predicate guidance from the taxonomy**: The extraction prompt's predicate standards are generated from the same DB taxonomy that generates the `remember` tool's `relationship_type` enum, grouped by their root domain with the leaf's own description as guidance. Governance changes to the vocabulary (adding, retiring, or retitling predicates) flow into the prompt automatically, and a test pins prompt and schema to the same list so the two cannot drift.
