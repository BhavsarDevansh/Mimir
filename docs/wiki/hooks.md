...
The pending queue lives in memory, so a daemon restart only loses work that has not started yet. Chat re-triggers on your next turn, connectors re-stage items from their durable sync cursor, and memory condensation re-triggers on the next fact write.
