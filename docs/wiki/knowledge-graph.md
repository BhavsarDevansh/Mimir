...
- **Performance:** WAL mode uses `synchronous=NORMAL`, a 40 MB per-connection page cache, and automatic `PRAGMA optimize` on close to keep local reads and writes responsive.
