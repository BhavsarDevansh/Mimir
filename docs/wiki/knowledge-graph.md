...
- The query tools resolve predicate filters through the alias-aware `get_relationship_type_id` lookup. Unknown predicates do not become relationship types: `kg_query` returns no matching facts, and `kg_related` skips that filter.
