...
Chat prompts use the budgeted rendering policy. It keeps the condensed core memory when it fits within the configured character limit, trims only the upcoming section to fill the remaining budget, and truncates core alone if that core is already too large; the system prompt then adds one request-local UTC time anchor. `/memory` uses full rendering so you can inspect all available memory content.
