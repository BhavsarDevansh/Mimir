...
Memory is regenerated **on demand** when facts change, not on a fixed timer. The `memory.condensation` background hook (debounced and idle-gated) ensures condensation only runs when the shared LLM worker pool is idle so it never slows down your conversations.
