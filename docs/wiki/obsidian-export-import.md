...
- **Large imports:** repeated shared objects use a per-import cache, and dry-run follows the same relationship-aware object resolution as apply (including renames within one import), so counts stay predictable on multi-file vaults.
- **Counts and vocabulary:** a fact line is counted as new only when its predicate is an emit-eligible taxonomy leaf and the exact triple is not already stored. Pending-confirmation facts count as new because dry-run and apply agree; invalid predicates appear in errors instead.
