...
| Calendar (Microsoft Graph) | 🟡 Partial | Outlook / Office 365 backend ([#474](https://github.com/BhavsarDevansh/Mimir/issues/474)): OAuth-only (PKCE loopback + connector-side refresh), events delta query with `$deltatoken` cursor, same fact cluster as CalDAV, `@removed` tombstones → KB fact lifecycle ([#247](https://github.com/BhavsarDevansh/Mimir/issues/247)), failure-safe cursor handoff ([#314](https://github.com/BhavsarDevansh/Mimir/issues/314)) plus delta-reset self-healing (expired tokens restart with a full sync). Read-only — no write-back. |
