...
| Relationship type DAG + aliases | ✅ Works | `relationship_type_hierarchy` + `relationship_type_aliases`; aliases resolve through the closed DB taxonomy ([#133](https://github.com/BhavsarDevansh/Mimir/issues/133)); conversational extraction resolves an emit-eligible DB leaf and rejects invented predicates instead of auto-creating rows ([#401](https://github.com/BhavsarDevansh/Mimir/issues/401)); the old public auto-creating relationship-type helper is removed, so new vocabulary requires seed migrations or explicit governance ([#559](https://github.com/BhavsarDevansh/Mimir/issues/559)); seeded subject/object entity-type constraints are enforced on every insert path (e.g. `born_on` requires a `DateTime` object) with unconstrained predicates and literal objects exempt ([#402](https://github.com/BhavsarDevansh/Mimir/issues/402)); redundant verbs are consolidated (`based_in`/`lived_in` → `resides_in`, `is_in` → `located_in`, `likes`/`loves`/`favourite_food`/`favourite_colour` → `prefers`) and the DAG is seeded with query-only roots for subtree queries ([#403](https://github.com/BhavsarDevansh/Mimir/issues/403)); every deterministic connector-emitted predicate is seeded canonical with subject/object constraints where applicable (typed entity-object shapes; literal-object predicates such as `took_photo` stay unconstrained) so a connector sync never auto-creates a predicate row, and email validates the DB-derived closed schema ([#412](https://github.com/BhavsarDevansh/Mimir/issues/412)). |
...
| Semantic entity dedup (LLM) | ✅ Implemented | The nightly `entity_semantic_dedup` pass evaluates capped candidate pairs (shared aliases, equal/contained names) with a strict LLM tool schema and queues them for review; `mimir kb merges list` shows pending suggestions and `apply` / `keep` resolve them ([#282](https://github.com/BhavsarDevansh/Mimir/issues/282)). |
| Pattern consolidation (nightly pass 8) | ❌ Not implemented | Pass logs "not yet implemented" and succeeds ([#67](https://github.com/BhavsarDevansh/Mimir/issues/67)). |
...
| Feature | Status | Notes & pending work |
|---------|--------|----------------------|
| Event overlay + scan job | ✅ Works | Lifecycle + recurrence overlay on facts; daily scan derives overlays, auto-completes past reminders, advances recurring events; "Upcoming" memory section ([#74](https://github.com/BhavsarDevansh/Mimir/issues/74)). The overlay carries the recurrence interval and effective series end (migration 058), so fortnightly events advance every two weeks and bounded series stop after their last occurrence. Superseded facts retire their overlay (dismissed + pending shape dropped) and the scan/render queries exclude superseded/forgotten facts, so a corrected recurring event surfaces exactly once ([#413](https://github.com/BhavsarDevansh/Mimir/issues/413)). |
...
| Calendar (Microsoft Graph) | 🟡 Partial | Outlook / Office 365 backend ([#474](https://github.com/BhavsarDevansh/Mimir/issues/474)): OAuth-only (PKCE loopback + connector-side refresh), events delta query with `$deltatoken` cursor, same fact cluster as CalDAV with recurrence interval + series bounds preserved, `@removed` tombstones → KB fact lifecycle ([#247](https://github.com/BhavsarDevansh/Mimir/issues/247)), failure-safe cursor handoff ([#314](https://github.com/BhavsarDevansh/Mimir/issues/314)) plus delta-reset self-healing (expired tokens restart with a full sync). Read-only — no write-back. |
...
| `GET` | `/kb/staged` | List unrecognized-predicate facts awaiting review (loopback only) |
| `POST` | `/kb/staged/{id}/map` | Map a staged fact to an emit-eligible leaf (loopback only) |
| `POST` | `/kb/staged/{id}/reject` | Reject a staged fact (loopback only) |
...
| Pattern consolidation (nightly pass 8) | [#67](https://github.com/BhavsarDevansh/Mimir/issues/67) |
