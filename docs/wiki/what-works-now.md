...
| Session compaction | ✅ Works | The `session.compaction` hook summarises old turns via the LLM, stores the summary on the session, and deletes the summarised messages; the summary is shown on `/history` resume and fed back into the model's context ([#279](https://github.com/BhavsarDevansh/Mimir/issues/279)). |
