...
Releases are published locally, never by CI: after the version bump, `scripts/new-release.sh` creates the annotated tag `vX.Y.Z`, pushes it to `origin`, and publishes a GitHub release whose body is auto-generated from the commits since the previous release (or `$RELEASE_NOTES_FILE` when set) (full walkthrough in [release-process.md](release-process.md)).
