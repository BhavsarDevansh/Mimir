...
/// extraction run: `accepted` counts facts inserted or saved as pending
/// sensitive confirmations, `dropped` counts rejected extraction or
/// normalization results, and `staged` counts durable unrecognized-predicate
/// records. The counts accumulate across backfill and incremental runs so
/// `mimir connector list` / `status` can surface the acceptance rate. Returns
...
pub async fn record_connector_fact_counts(
    ...
    if accepted < 0 || dropped < 0 || staged < 0 {
        return Err(KnowledgeError::Validation(format!(
            "connector fact counters must be non-negative (accepted: {accepted}, dropped: {dropped}, staged: {staged})"
        )));
    }
    let result = sqlx::query(
        "UPDATE connectors SET facts_accepted = facts_accepted + ?, \
         facts_dropped = facts_dropped + ?, facts_staged = facts_staged + ?, updated_at = ? WHERE id = ?",
    )
    .bind(accepted)
    .bind(dropped)
    .bind(staged)
