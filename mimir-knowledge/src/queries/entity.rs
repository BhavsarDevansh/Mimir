...
// ---------------------------------------------------------------------------
// Proximity query (Phase 3 S4 / issue #194)
// ---------------------------------------------------------------------------

/// Find entity locations within `radius_km` of `(lat, lon)`, sorted
/// nearest-first.
///
/// Two-stage strategy (per the issue spec): a coarse SQLite bounding-box
/// pre-filter over `latitude`/`longitude` (backed by the composite index from
/// migration 045), then an exact great-circle distance computed in Rust via
/// [`geo::haversine_km`] that drops edge-of-box points outside the radius and
/// sorts the survivors. Locations with NULL coordinates are excluded by the
/// `IS NOT NULL` predicates (and are not present in the coordinate index).
///
/// When `at` is `Some(t)`, only locations whose temporal bounds contain `t`
/// are considered — `valid_from IS NULL OR valid_from <= t` **and**
/// `valid_until IS NULL OR valid_until >= t`. `None` performs a pure spatial
/// query over all locations regardless of validity, which suits historical
/// `Visited`/`Origin` overlays.
///
/// The SQL `ORDER BY` is deliberately omitted: the survivor set is small after
/// the radius post-filter, and the authoritative order is the Rust Haversine
/// distance, so sorting in Rust avoids a redundant SQLite sort and keeps the
/// distance computation single-source.
