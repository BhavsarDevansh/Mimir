...
/// Build a daemon HTTP client for the resolved transport (Unix socket or
/// TCP), authenticating with the local API token (issue #281). The token is
/// loaded — or generated — from the data dir, so the CLI works unmodified
/// after `mimir init` and can even create the token before auto-starting the
/// daemon. If the token cannot be loaded or attached as a header, a warning
/// is printed and a tokenless client is returned so the daemon's `401`
/// surfaces the problem instead of a panic.
pub fn make_client(transport: &DaemonTransport) -> MimirClient {
    let token = match mimir_core::auth::load_or_create_api_token() {
        ...
        Err(error) => {
            ...
            return tokenless_client(transport);
        }
    };
    client_with_token(transport, token)
}
...
fn client_with_token(transport: &DaemonTransport, token: String) -> MimirClient {
    match transport {
        ...
        #[cfg(unix)]
        DaemonTransport::Unix(socket_path) => {
            match MimirClient::try_new_with_token_uds(
                socket_path,
                token,
                MimirClient::DEFAULT_CONNECT_TIMEOUT,
                MimirClient::DEFAULT_TOTAL_TIMEOUT,
                ...
                Err(error) => {
                    ...
                    MimirClient::new_uds(socket_path)
                }
            }
        }
    }
...
/// Build a tokenless client for the resolved transport.
fn tokenless_client(transport: &DaemonTransport) -> MimirClient {
    match transport {
        DaemonTransport::Tcp(base_url) => MimirClient::new(base_url),
        #[cfg(unix)]
        DaemonTransport::Unix(socket_path) => MimirClient::new_uds(socket_path),
    }
}
