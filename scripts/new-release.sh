...
# version from the workspace manifest (or an explicit VERSION argument), creates
# an annotated tag, pushes it, and runs `gh release create`. The release body is
# generated by GitHub from the commits since the previous release, or read from
# `$RELEASE_NOTES_FILE` when that is set. A real release requires a clean working
# tree and an authenticated `gh` CLI; a failed publication can be retried and
# resumes from the already-pushed tag.
#
...
Environment:
  ...
  RELEASE_NOTES_FILE  Optional file with hand-written release notes; when unset,
                      GitHub generates the notes from the commits since the
                      previous release

The script will:
  1. Read VERSION (argument or workspace manifest)
  2. Validate local/remote tag state and a clean working tree
  3. Check `gh` authentication, then create and push the annotated tag
     vVERSION (skipped when already present)
  4. Create the GitHub release, using the `RELEASE_NOTES_FILE` body when set
     or GitHub-generated notes otherwise
USAGE
}
...
# ---------------------------------------------------------------------------
main() {
    local version="" url notes_source
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --dry-run)
                DRY_RUN=true
    ...
    if [[ -n "$RELEASE_NOTES_FILE" ]]; then
        ...
        notes_source="from $RELEASE_NOTES_FILE"
    else
        notes_source="GitHub-generated (commits since the previous release)"
    fi
    ...
    if [[ -n "$RELEASE_NOTES_FILE" ]]; then
        url="$(gh release create "$tag" --title "$tag" --notes-file "$RELEASE_NOTES_FILE")"
    else
        url="$(gh release create "$tag" --title "$tag" --generate-notes)"
    fi
    info "Released: $url"
}
